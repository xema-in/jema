    ...
    */
    ...
    private setupSignalR(): void {
        ...
        this.connection = new signalR.HubConnectionBuilder()
            .withUrl(`${this.backendUrl}/agents?access_token=${this.token}`).build();
    ...
    private processConferenceEvents(event: string, message: any) {
        switch (event) {
            case 'ConfbridgeStart':
                ...
                break;
            case 'ConfbridgeJoin':
                if (this.conferenceCallCache !== undefined && this.conferenceCallCache.members.length < 2) {
    ...
    private processTeamMemberState(message: any) {
        ...
        let currentStatus = this.teamMemberStatesCache.find(x => x.agentId === message.agentId);
        if (currentStatus === undefined) {
            ...
            currentStatus = this.teamMemberStatesCache.find(x => x.agentId === message.agentId);
        }
    ...
    private processIdCard(message: any) {
        const currentStatus = this.teamMemberStatesCache.find(x => x.agentId === message.userName);
        if (currentStatus !== undefined) {
